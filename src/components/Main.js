...
function Main() {
  ...
  useEffect(() => {
    (async function () {
      try {
        ...
        if (!data.ok) throw new Error("Failed to reach the data");
  ...
  return (
    <main>
      {status === "loading" && <p id="loading"> loading questions...</p>}
      {status === "error" && <p id="error">There was an error fetching the questions :(</p>}
